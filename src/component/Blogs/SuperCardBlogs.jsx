...
import React, { useEffect, useState } from 'react';
import axios from 'axios';
// تم حذف جميع استيرادات Swiper لأننا سنستخدم Grid Layout
...
const SuperNovaCard = () => {
  ...
  return (
    <div className="container mx-auto py-10 px-4">
      {services.length > 0 ? (
        // استخدام Tailwind CSS لإنشاء Grid متجاوب
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {services.map((service) => (
            <div key={service.id} className="rounded-xl overflow-hidden text-black shadow-lg hover:shadow-2xl transition-shadow duration-300">
              {/* عرض الصورة الرئيسية فقط بدلاً من السلايدر الداخلي */}
              <img
              ...
              <div className="p-4 space-y-2">
                ...
                <p className="text-sm text-gray-300 text-center break-words overflow-hidden whitespace-normal">{service.short_description}</p>
              </div>
            </div>
          ))}
        </div>
